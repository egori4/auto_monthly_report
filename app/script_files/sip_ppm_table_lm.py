...
def df():
	...
	for month in month_list:
		...
		df = pd.DataFrame(pd.read_sql_query("select sourceAddress as 'Source IP',sum(packetCount)/1000000 as "+cal_month+" from attacks where month="+str(month)+" group by sourceAddress order by sum(packetCount) desc", con)).set_index('Source IP')
		frames.append(df)
...
def AbuseIPDBGEO(ip):
	with open('./script_files/AbuseIPDB/Raw Data/AbuseIPDBPkts.json') as abuseipdb_file:
