...
def df():
	...
	for month in month_list:
		...
		df = pd.DataFrame(pd.read_sql_query("select sourceAddress as 'Source IP',count(*) as "+cal_month+" from attacks where month="+str(month)+" group by sourceAddress order by count(*) desc", con)).set_index('Source IP')
		frames.append(df)
...
def AbuseIPDBGEO(ip):
	with open('./script_files/AbuseIPDB/Raw Data/AbuseIPDBEvts.json') as abuseipdb_file:
